  ...
  return (
    <header className="w-full border-b border-[#1c1b19] bg-[#0b0b0b]/90 backdrop-blur-sm sticky top-0 z-50 px-4 sm:px-6 lg:px-12 py-3.5">
      <div className="w-full flex items-center justify-between font-mono text-xs tracking-[0.15em] text-[#918c84]">
        ...
        {/* Nav Links: HOME -> ABOUT -> WORK -> SKILLS (#skills) -> CONTACT */}
        <nav className="hidden md:flex items-center gap-5 lg:gap-7 text-xs font-mono font-bold tracking-[0.15em]">
          ...
          <a href="#work" className="text-[#ded8ce] hover:text-amber-400 transition-colors uppercase">
            WORK & CREATIONS
          </a>
          <a href="#skills" className="text-[#ded8ce] hover:text-amber-400 transition-colors uppercase">
            SKILLS
          </a>
        ...
        {/* Right Action Bar: Resume Button & Status */}
      ...
      {isMobileMenuOpen && (
        <nav className="md:hidden mt-3 pt-3 border-t border-[#262420] flex flex-col gap-2.5 font-mono text-xs font-bold tracking-[0.15em]">
          <a href="#hero" onClick={() => setIsMobileMenuOpen(false)} className="py-1 text-[#ded8ce] hover:text-amber-400 uppercase">HOME</a>
          <a href="#about" onClick={() => setIsMobileMenuOpen(false)} className="py-1 text-[#ded8ce] hover:text-amber-400 uppercase">ABOUT</a>
          <a href="#work" onClick={() => setIsMobileMenuOpen(false)} className="py-1 text-[#ded8ce] hover:text-amber-400 uppercase">WORK & CREATIONS</a>
          <a href="#skills" onClick={() => setIsMobileMenuOpen(false)} className="py-1 text-[#ded8ce] hover:text-amber-400 uppercase">SKILLS</a>
          <a href="#contact" onClick={() => setIsMobileMenuOpen(false)} className="py-1 text-[#ded8ce] hover:text-amber-400 uppercase">CONTACT</a>
        </nav>
      )}
    </header>
  );
